...
def calendar_heatmap(data):
    """
    달력형 히트맵을 생성하여 출발 날짜(depart_time_dep)별 평균 요금을 시각화
    """
    # 1. 날짜 정보 변환
    ...
    for month in sorted(months):
        ...
        # 4. 요금 데이터를 달력에 매핑
        fare_values = np.full_like(month_calendar, np.nan, dtype=float)  # 요금을 저장할 배열
            ...
            for day_idx, day in enumerate(week):
                if day > 0:  # 유효한 날짜만 처리
                    ...
                    if not day_data.empty: # 필터링된 데이터가 비어 있는지 확인
                        fare = day_data['fare'].mean()  # 평균 요금 계산
                        weekday_idx = day_data['weekday'].iloc[0]  # 요일 인덱스
                        weekday = calendar.day_abbr[weekday_idx]  # 요일 인덱스를 요일 약자로 변환환
                        fare_values[week_idx][day_idx] = fare # (해당 주, 해당 날짜)의 평균 요금 저장
        ...
        ax.set_title(f'{year}년 {month}월 항공권 평균 요금', fontsize=16, fontweight='bold')
        ax.axis('off')
