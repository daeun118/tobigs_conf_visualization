...
def airline_bar_graph(data):
    ...
    axes[0].set_ylim(0, avg_sorted['avg_fare'].max() * 1.2) # y축 범위 조정
    axes[0].grid(axis='y', linestyle='--', alpha=0.6)

    # 평균 요금 값 표시
    for index, row in avg_sorted.iterrows():
        axes[0].text(index, row['avg_fare'] + 1000, f"{int(row['avg_fare']):,}", ha='center', fontsize=10, fontweight='bold')
    ...
    axes[1].set_ylim(0, median_sorted['median_fare'].max() * 1.2) # y축 범위 조정
    axes[1].grid(axis='y', linestyle='--', alpha=0.6)

    # 중앙값 요금 값 표시
    for index, row in median_sorted.iterrows():
        axes[1].text(index, row['median_fare'] + 1000, f"{int(row['median_fare']):,}", ha='center', fontsize=10, fontweight='bold')

    # 6. X축 설정 (각각 독립)
    axes[0].set_xticks(range(len(avg_sorted['airline'])))  # 고정된 눈금 설정
    axes[0].set_xticklabels(avg_sorted['airline'], rotation=45, fontsize=10)
    axes[1].set_xticks(range(len(median_sorted['airline'])))  # 고정된 눈금 설정
